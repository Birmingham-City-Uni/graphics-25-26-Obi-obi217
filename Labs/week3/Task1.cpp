	...
	uint8_t r, uint8_t g, uint8_t b, uint8_t a = 255)
{
	...
	for(int x = minX; x <= maxX; ++x) 
		for (int y = minY; y <= maxY; ++y) {
			...
			float a0 = (Vector2 (p0.x() - p.x(), p0.y() - p.y()).cross(Vector2 (p1.x() - p.x(), p1.y() - p.y())))/2;
			float a1 = (Vector2 (p1.x() - p.x(), p1.y() - p.y()).cross(Vector2 (p2.x() - p.x(), p2.y() - p.y())))/2;
			float a2 = (Vector2 (p2.x() - p.x(), p2.y() - p.y()).cross(Vector2 (p0.x() - p.x(), p0.y() - p.y())))/2;
			...
			if (sum <= 1.01f) { setPixel(image, x, y, width, height, r, g, b, a); }
		}
}
...
int main()
{
	...
	for (const auto& face : faces) {
		...
		drawTriangle(imageBuffer, width, height, p0, p1, p2, rand() % 256, rand() % 256, rand() % 256, rand() % 256);
